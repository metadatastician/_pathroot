...
const VERSION = "0.1.0";
const BUILD_INFO = "_PATHROOT built with Zig " ++ @import("builtin").zig_version_string;
...
export fn _pathroot_init() ?*Handle {
...
export fn _pathroot_free(handle: ?*Handle) void {
...
export fn _pathroot_process(handle: ?*Handle, input: u32) Result {
...
export fn _pathroot_get_string(handle: ?*Handle) ?[*:0]const u8 {
    const h = handle orelse {
        setError("Null handle");
        return null;
    };
...
export fn _pathroot_free_string(str: ?[*:0]const u8) void {
...
export fn _pathroot_process_array(
    handle: ?*Handle,
    buffer: ?[*]const u8,
    len: u32,
) Result {
...
export fn _pathroot_last_error() ?[*:0]const u8 {
...
export fn _pathroot_version() [*:0]const u8 {
    return VERSION.ptr;
}

/// Get build information
export fn _pathroot_build_info() [*:0]const u8 {
...
/// Register a callback
export fn _pathroot_register_callback(
    handle: ?*Handle,
    callback: ?Callback,
) Result {
    const h = handle orelse {
...
export fn _pathroot_is_initialized(handle: ?*Handle) u32 {
...
test "lifecycle" {
    ...
    try std.testing.expect(_pathroot_is_initialized(handle) == 1);
}

test "error handling" {
    const result = _pathroot_process(null, 0);
