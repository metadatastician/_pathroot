...
// Replace rsr-template-repo, _pathroot, None with actuals
...
| `$PREFIX/share/_pathroot/`
| Data files, assets

| `$PREFIX/share/doc/_pathroot/`
| Documentation
...
Default config location: `$XDG_CONFIG_HOME/_pathroot/config.toml`

Fallback: `$HOME/.config/_pathroot/config.toml`
...
Or via OPSM: `opsm update _pathroot`
...
[source,bash]
----
just install --prefix=/opt/_pathroot-instance1 --config=/etc/_pathroot/instance1.toml
just install --prefix=/opt/_pathroot-instance2 --config=/etc/_pathroot/instance2.toml
----
